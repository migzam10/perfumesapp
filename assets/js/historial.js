...
async function loadHistorial() {
  ...
  el.innerHTML = Object.entries(byDate).map(([fecha,vs])=>{
    const total = vs.reduce((s,v)=> s + parseFloat(v.total || 0), 0);
    const rows  = vs.map(v=>`
      <div class="vi">
        <div class="vi-hdr">
          ...
          <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px">
            <div class="vi-total">${fmt(Math.round(v.total))}</div>
    ...
    return `<div style="margin-bottom:14px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:7px">
        ...
        <span style="font-size:.78rem;color:var(--gold);font-weight:600">${fmt(Math.round(total))}</span>
      </div>
      ${rows}
    </div>`;
  }).join('');
